...
function AdminSeoFallback() {
  return (
    <p className="admin-empty" aria-live="polite">
      Loading SEO advisory…
    </p>
  );
}

async function AdminSeoSuggestionsLoader({
  params,
}: {
...
export default function AdminEditorialSeoSuggestionsPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  return (
    <div className="admin-shell admin-shell-root">
      ...
      <main className="admin-shell-main">
        <Suspense fallback={<AdminSeoFallback />}>
          <AdminSeoSuggestionsLoader params={params} />
        </Suspense>
      </main>
    </div>
  );
